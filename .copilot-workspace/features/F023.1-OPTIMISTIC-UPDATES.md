...
const { incrementResponseCount, revertResponseCount, refetchTask } = useOptimisticTaskUpdate()

// Replace handleResponseSubmitted
const handleResponseSubmitted = async (responseData: unknown): Promise<void> => {
  try {
    ...
    submissionError.value = error instanceof Error ? error.message : 'Unknown error'
  }
}
