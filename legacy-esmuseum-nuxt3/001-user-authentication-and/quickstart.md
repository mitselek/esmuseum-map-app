...
2. **Test Data Requirements**:
   - Entu person entity with group enrollment
   - Entu person entity without group enrollment (non-student)
...
**User Story**: _As a student, I want to authenticate with Entu using OAuth so that I can access Museum School programs with my existing enrollment._
...
3. **Complete Entu Authentication**:
   ...
   ```text
   Action: Follow Entu OAuth authentication process
...
5. **Validate Estonian Student Detection**:
   ...
   ```javascript
   const profile = JSON.parse(localStorage.getItem('esmuseum_user_profile'));
   console.log('Student profile:', profile.person);
   // Expected: Valid student profile with group enrollment
   ```
...
- ✅ Student authentication via OAuth functional
- ✅ Student enrollment detected via group membership
...
2. **Verify International Student Detection**:
   ...
   ```javascript
   const profile = JSON.parse(localStorage.getItem('esmuseum_user_profile'));
   const hasGroupEnrollment = profile.person._parent.length > 0;
   console.log('Student type:', hasGroupEnrollment ? 'ENROLLED' : 'NOT_ENROLLED');
   // Expected: 'NOT_ENROLLED' for users without group membership
   ```
