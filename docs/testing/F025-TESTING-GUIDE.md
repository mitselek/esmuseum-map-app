...
console.log('Token expires in:', Math.floor((payload.exp * 1000 - Date.now()) / 1000), 'seconds');
...
const newPayload = btoa(JSON.stringify(payloadObj)).replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_');
...
function checkToken() {
  ...
  try {
    ...
    console.log('Status:', remaining > 60000 ? '✅ Valid' : '⚠️ Expiring soon (< 60s)');
...
function checkAuthStorage() {
  ...
  console.log('Token:', localStorage.getItem('esm_token') ? '✅ Present' : '❌ Missing');
  console.log('User:', localStorage.getItem('esm_user') ? '✅ Present' : '❌ Missing');
